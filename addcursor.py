...
class AddCursorTrailFrame(customtkinter.CTkFrame, TkinterDnD.DnDWrapper):
    def __init__(self, *args, header_name="Add cursor", **kwargs):
        super().__init__(*args, **kwargs)
        self.TkdndVersion = TkinterDnD._require(self)
    ...
    def get_path(self, event):
        ...
        self.file = str(self.file).replace('}', '')
        image = customtkinter.CTkImage(light_image=Image.open(fp=self.file), size=[25, 25])
        self.label2.grid(row=1, column=1, padx=(35, 0))
        self.label2.configure(text="", image=image)
    ...
    def add_cursor(self):
        ...
        self.button_apply.destroy()
        self.button_cancel.destroy()
        CursorTrailFrame.update_file_list(self, f"{APPDIR}\\images\\cursortrail\\cursortrail (1).png")
